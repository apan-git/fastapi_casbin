...
from apps.user.schems import CreateUserResp, UserCreate, UserPhone
...
async def register(*, phone: int, db: Session = Depends(deps.get_db)) -> Any:
    user = crud_user.get_by_phone(db, phone=phone)
    if user:
        resp_user = crud_user.update_code(db, phone)
    else:
        user_info = UserCreate(phone=phone)
